...
public class RecycleSideDish extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getitems_usingVolley("get_enabled_items", "4", list);
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL_PROCESSING,
                        ...
                        try {
                            ...
                            if (response_status.equals("success")) {
                                for (int i = 0; i < jsonArray_response_data.length(); i++) {
                                    ...
                                    eachitem = new itemStore(item_image, item_name, item_price, item_auto_id, "SideDish");
                                    list1.add(eachitem);

                                }
                                adapter1 = new MyRecyclerAdapter(RecycleSideDish.this, list1);
                                rec.setAdapter(adapter1);
                                ...
                                Toast.makeText(RecycleSideDish.this, "No items !!!", Toast.LENGTH_SHORT).show();
                                //Log.v("response", "Invalid Credentials");
                            }
                        } catch (Exception ex) {
                            //show_dialog_message("Network Error", "Check network connection");
                            Toast.makeText(RecycleSideDish.this, "Network Error!!!", Toast.LENGTH_SHORT).show();
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        //progressDialog.dismiss();
                        //show_dialog_message("Network Error", "Check network connection");
                        Toast.makeText(RecycleSideDish.this, "Network Error2", Toast.LENGTH_SHORT).show();
